				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("key.tableName") === oObject.key.tableName;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].key.tableName;
										} else {
											sCurrentId = aItemsNotInTheList[0].key.tableName;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("tables", {
											key.tableName : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("key.tableName"),
							title: oBindingContext.getProperty("key.tableName")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "tables (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'tables (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("key.tableName");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all tables to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "tables (" + iExpectedLength + ")"}),
									success : function () {
